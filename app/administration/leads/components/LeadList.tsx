...
export function LeadList({ 
  ...
  const table = useReactTable({
    ...
    // On retire la logique de pagination de tanstack/react-table
  });
  ...
  return (
    <div className="rounded-lg border border-gray-200 dark:border-dark-border bg-white dark:bg-dark-surface shadow-sm">
        ...
        <table className="w-full text-sm">
          <thead className="[&_tr]:border-b">
            {table.getHeaderGroups().map((headerGroup: HeaderGroup<LeadWithAssignedUser>) => (
              <tr key={headerGroup.id} className="border-b transition-colors hover:bg-gray-100/50 dark:hover:bg-white/5">
            ...
            {table.getRowModel().rows?.length ? (
              table.getRowModel().rows.map((row: Row<LeadWithAssignedUser>) => (
                <tr
                  ...
                  className="border-b transition-colors hover:bg-gray-100/50 data-[state=selected]:bg-gray-100 dark:hover:bg-white/5 dark:data-[state=selected]:bg-dark-border"
                >
      ...
      {/* On passe les bonnes props au composant de pagination */}
      <PaginationControls 
