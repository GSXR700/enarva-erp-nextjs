...
import { Role, Employee, PayRate, EmployeeType, Department, User } from "@prisma/client";
...
const PasswordStrengthIndicator = ({ strength }: { strength: { score: number; label: string; color: string; } }) => {
    if (strength.score === 0) return null;
    return (
        <div className="flex items-center gap-2 mt-1">
            <div className="w-full bg-gray-200 dark:bg-dark-background rounded-full h-1.5">
                <div 
...
export function EmployeeFormModal({ isOpen, onClose, employee, payRates, departments }: EmployeeFormModalProps) {
  ...
  useEffect(() => {
    const checkPasswordStrength = (pass: string) => {
      let score = 0;
      if (!pass) { setPasswordStrength({ score: 0, label: '', color: 'bg-transparent' }); return; }
      if (pass.length >= 8) score++;
      if (/[a-z]/.test(pass)) score++;
      if (/[A-Z]/.test(pass)) score++;
      if (/[0-9]/.test(pass)) score++;
      if (/[^a-zA-Z0-9]/.test(pass)) score++;
      let label = 'Faible'; let color = 'bg-red-500';
      if (score > 2) { label = 'Moyen'; color = 'bg-orange-500'; }
      if (score > 4) { label = 'Fort'; color = 'bg-green-500'; }
      setPasswordStrength({ score: score > 4 ? 4 : score, label, color });
    };
  ...
  useEffect(() => {
    if (isOpen) {
      reset({
        ...
        role: employee?.user?.role || Role.FIELD_WORKER,
        type: employee?.type || EmployeeType.JOURNALIER,
          ...
          <div className="p-6 space-y-4 max-h-[80vh] overflow-y-auto">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                    ...
                    <input id="firstName" {...register("firstName")} className="w-full p-2 border rounded bg-gray-50 dark:bg-dark-background dark:border-dark-border text-gray-800 dark:text-dark-text" />
                ...
                <div>
                    ...
                    <input id="lastName" {...register("lastName")} className="w-full p-2 border rounded bg-gray-50 dark:bg-dark-background dark:border-dark-border text-gray-800 dark:text-dark-text" />
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                    ...
                    <input id="email" type="email" {...register("email")} className="w-full p-2 border rounded bg-gray-50 dark:bg-dark-background dark:border-dark-border text-gray-800 dark:text-dark-text" />
                ...
                <div>
                    ...
                    <input id="phone" {...register("phone")} className="w-full p-2 border rounded bg-gray-50 dark:bg-dark-background dark:border-dark-border text-gray-800 dark:text-dark-text" />
                </div>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div>
                ...
                <select id="type" {...register("type")} className="w-full p-2 border rounded bg-gray-50 dark:bg-dark-background dark:border-dark-border text-gray-800 dark:text-dark-text">
               ...
               <div>
                ...
                <select id="departmentId" {...register("departmentId")} className="w-full p-2 border rounded bg-gray-50 dark:bg-dark-background dark:border-dark-border text-gray-800 dark:text-dark-text">
            ...
            {/* AFFICHEGE CONDITIONNEL POUR LES EMPLOYES CDI */}
            {contractType === 'CDI' && (
              <div className="p-4 my-2 border-l-4 border-primary bg-primary-light dark:bg-dark-surface space-y-4 rounded-r-md">
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                      <div>
                          ...
                          <input id="salaireDeBase" type="number" step="0.01" {...register("salaireDeBase")} placeholder="0.00" className="w-full p-2 border rounded bg-white dark:bg-dark-background dark:border-dark-border text-gray-800 dark:text-dark-text" />
                      </div>
                       <div>
                          ...
                          <select id="defaultPayRateId" {...register("defaultPayRateId")} className="w-full p-2 border rounded bg-white dark:bg-dark-background dark:border-dark-border text-gray-800 dark:text-dark-text">
                   ...
                   <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                      <div>
                          ...
                          <input id="numeroCNSS" {...register("numeroCNSS")} className="w-full p-2 border rounded bg-white dark:bg-dark-background dark:border-dark-border text-gray-800 dark:text-dark-text" />
                      </div>
                       <div>
                          <label htmlFor="numeroCIN" className="block mb-1 text-sm font-medium text-gray-700 dark:text-dark-subtle">N° CIN</label>
                          <input id="numeroCIN" {...register("numeroCIN")} className="w-full p-2 border rounded bg-white dark:bg-dark-background dark:border-dark-border text-gray-800 dark:text-dark-text" />
                      </div>
                  </div>
              </div>
            )}
            ...
            {contractType !== 'CDI' && (
              <div>
                ...
                <select id="defaultPayRateId" {...register("defaultPayRateId")} className="w-full p-2 border rounded bg-gray-50 dark:bg-dark-background dark:border-dark-border text-gray-800 dark:text-dark-text">
                  <option value="">-- Non spécifié --</option>
                  {payRates.map(rate => ( <option key={rate.id} value={rate.id}>{rate.name}</option> ))}
                </select>
              </div>
            ...
            <div>
                ...
                <select id="role" {...register("role")} className="w-full p-2 border rounded bg-gray-50 dark:bg-dark-background dark:border-dark-border text-gray-800 dark:text-dark-text">
            ...
            <div className="pt-4 border-t dark:border-dark-border">
                <p className="text-sm text-gray-500 dark:text-dark-subtle">{employee ? "Modifier le mot de passe (optionnel)" : "Définir un mot de passe"}</p>
                 <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mt-2">
                    <div className="relative">
                        ...
                        <input id="password" type={showPassword ? "text" : "password"} {...register("password")} placeholder={employee ? "Laisser vide pour ne pas changer" : ""} className="w-full p-2 border rounded bg-gray-50 dark:bg-dark-background dark:border-dark-border text-gray-800 dark:text-dark-text pr-10" />
                        <button type="button" onClick={() => setShowPassword(!showPassword)} className="absolute inset-y-0 right-0 top-6 flex items-center px-3 text-gray-500 dark:text-dark-subtle"><EyeOff size={16} /></button>
                    </div>
                    <div>
                        ...
                        <input id="confirmPassword" type={showPassword ? "text" : "password"} {...register("confirmPassword")} className="w-full p-2 border rounded bg-gray-50 dark:bg-dark-background dark:border-dark-border text-gray-800 dark:text-dark-text" />
                    </div>
                </div>
